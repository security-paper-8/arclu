...
def light_imagenet_exp_linf_targeted(ROOT_PATH, baselines):
    attack = "pgd"
    attack_args = PGDParam("light_imagenet", "linf", 0.03,
...
def light_imagenet_exp_l2_targeted(ROOT_PATH, baselines):
    attack = "pgd"
    attack_args = PGDParam("light_imagenet", "l2", 6.0,
                           0.1, 1000, attack_size=500)
    attack_args["targeted"] = ""
    EXP_PATH = "light_imagenet_{}_{}_{}_{}".format(attack,
                                                   "targeted" if "targeted" in attack_args.keys() else "untargeted",
                                                   attack_args["eps_type"],
...
def light_imagenet_exp_l2_untargeted(ROOT_PATH, baselines):
    attack = "pgd"

    attack_args = PGDParam("light_imagenet", "l2", 6.0,
                           0.2, 1000, attack_size=500)
    EXP_PATH = "light_imagenet_{}_{}_{}_{}".format(attack,
                                                   "targeted" if "targeted" in attack_args.keys() else "untargeted",
                                                   attack_args["eps_type"],
                                                   attack_args["eps"])
...
def light_imagenet_exp():
    ...
    executor_list.extend(light_imagenet_exp_l2_targeted(ROOT_PATH, baselines))
    executor_list.extend(light_imagenet_exp_l2_untargeted(ROOT_PATH, baselines))
