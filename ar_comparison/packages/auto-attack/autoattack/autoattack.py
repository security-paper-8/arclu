...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, n_target_classes=9, threshold_each_class=None):
        self.model = model
        self.norm = norm
        ...
        if not self.is_tf_model:
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                                                     eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device)
        self.apgd.threshold_each_class = threshold_each_class
        self.fab.threshold_each_class = threshold_each_class
        self.square.threshold_each_class = threshold_each_class
        self.apgd_targeted.threshold_each_class = threshold_each_class
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                x = x_orig[start_idx:end_idx, :].clone().to(self.device).float()
            ...
            x_adv = x_orig.clone().detach().float()
            startt = time.time()
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    x = x_orig[batch_datapoint_idcs, :].clone().to(
                        self.device).float()
                    y = y_orig[batch_datapoint_idcs].clone().to(self.device)

                    # make sure that x is a 4d tensor even if there is only a single datapoint left
                    # BG: we only ensure the dimension is valid
                    # if len(x.shape) == 3:
                    #    x.unsqueeze_(dim=0)
                    ...
                    if attack == 'apgd-ce':
                        ...
                        print(",,,,,", x.shape)
                        _, adv_curr = self.apgd_targeted.perturb(
                            x, y, cheap=True)
                    ...
                    false_batch = ~y.eq(output.max(dim=1)[1]).to(
                        robust_flags.device)
                ...
                robust_accuracy = torch.sum(
                    robust_flags).item() / x_orig.shape[0]
                if self.verbose:
            ...
            if self.verbose:
                if self.norm == 'Linf':
                    res = (
                        x_adv - x_orig).abs().view(x_orig.shape[0], -1).max(1)[0]
                elif self.norm == 'L2':
                    res = ((x_adv - x_orig) **
                           2).view(x_orig.shape[0], -1).sum(-1).sqrt()
    ...
    def clean_accuracy(self, x_orig, y_orig, bs=250):
        ...
        for counter in range(n_batches):
            x = x_orig[counter * bs:min((counter + 1) * bs,
                                        x_orig.shape[0])].clone().to(self.device).float()
            y = y_orig[counter * bs:min((counter + 1) *
                                        bs, x_orig.shape[0])].clone().to(self.device)
            output = self.get_logits(x)
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            # , 'apgd-t' ['apgd-dist', 'apgd-ce', 'apgd-t', 'fab-t', 'square']#'apgd-dist', ['apgd-ce', 'apgd-t']#
            self.attacks_to_run = ['apgd-dist',
                                   'apgd-ce', 'apgd-t', 'fab-t', 'square']
            #self.attacks_to_run = [ 'apgd-t', 'fab-t', 'apgd-dist', 'apgd-ce', 'square']
            #self.attacks_to_run = ['square', 'fab-t', 'apgd-dist', 'apgd-ce', 'apgd-t']
            self.apgd.n_restarts = 1
            self.fab.n_restarts = 1
            self.apgd_targeted.n_restarts = 1
            self.fab.n_target_classes = self.n_target_classes
            self.apgd_targeted.n_target_classes = self.n_target_classes
            self.square.n_queries = 5000

        elif version == 'plus':
            self.attacks_to_run = ['apgd-ce', 'apgd-dlr',
                                   'fab', 'square', 'apgd-t', 'fab-t']
