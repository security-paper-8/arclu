...
class APGDAttack():
    ...
    def distance_loss(self, x, y):
        # Above loss is changed to loss below.
        # Since above loss is not dealing with absolute value of logits (in our case distance to a class center) and considers
        # relative ratio between logits, it was not effective in attacks.

        # cw loss
        true_one_hot = torch.eye(x.size(1))[y].cuda()
        other_one_hot = 1 - true_one_hot
        true_logit = (x * true_one_hot).sum(dim=1)
        other_logit = (x * other_one_hot - true_one_hot *
                       999999999999999).max(dim=1)[0]
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        self.n_iter_2, self.n_iter_min, self.size_decr = max(int(
            0.22 * self.n_iter), 1), max(int(0.06 * self.n_iter), 1), max(int(0.03 * self.n_iter), 1)
        if self.verbose:
            print('parameters: ', self.n_iter, self.n_iter_2,
        ...
        if self.loss == 'ce':
            # nn.CrossEntropyLoss(reduce=False, reduction='none')
            criterion_indiv = self.distance_loss
        elif self.loss == 'dist':
            # nn.CrossEntropyLoss(reduce=False, reduction='none')
            criterion_indiv = self.distance_loss
        elif self.loss == 'dlr':
        ...
        for i in range(self.n_iter):
            ...
            pred_logits, pred_class = logits.detach().max(1)
            pred = pred_class == y
            #print("selected tec:",self.threshold_each_class[pred_class.cpu().numpy()])
            print("current value:", pred_logits)
        ...
        if not best_loss:
            ...
            if not cheap:
                ...
                for counter in range(self.n_restarts):
                    ...
                    #ind_to_fool = torch.arange(acc.size(0))
...
class APGDAttack_targeted():
    ...
    def distance_loss_targeted(self, x, y, y_target):
        true_one_hot = torch.eye(x.size(1))[y].cuda()
        true_logit = (x * true_one_hot).sum(dim=1)
        other_logit = x[torch.arange(x.size(0)), y_target]
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for i in range(self.n_iter):
            ...
            x_best_adv[(pred == 0).nonzero().squeeze()] = (
                x_adv[(pred == 0).nonzero().squeeze()] + 0.).detach()
            if self.verbose:
                print('iteration: {} - Best loss: {:.6f}'.format(i, loss_best.sum()))
            # x_adv: current perturbed adversarial example
            # x_best_adv: current perturbed 'misleading' adversarial example
            # x_best: highest-loss adversarial example
        ...
        if not best_loss:
            ...
            for target_class in range(2, self.n_target_classes + 2):
                ...
                for counter in range(self.n_restarts):
                    ...
                    if self.verbose:
                        print(
                            'restart {} - loss: {:.5f}'.format(counter, loss_best.sum()))
