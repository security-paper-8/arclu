...
class ModelAdapter():
    def __init__(self, logits, x, y, sess, num_classes=10):
        ...
        if num_classes <= 10:
            ...
            for cl in range(num_classes):
                self.grads[cl] = tf.gradients(
                    self.logits[:, cl], self.x_input)[0]
        ...
        # dist loss
        self.dist = self.distance_loss(
            logits=self.logits, y=self.y_input, num_classes=self.num_classes)
        self.grad_dist = tf.gradients(self.dist, self.x_input)[0]
        ...
        self.dlr_target = dlr_loss_targeted(
            self.logits, self.y_input, self.y_target, num_classes=self.num_classes)
    ...
    def distance_loss(self, logits, y, num_classes):
        true_one_hot = tf.one_hot(y, num_classes)
        other_one_hot = 1 - true_one_hot
        other_logit = tf.reduce_max(
            logits * other_one_hot - true_one_hot * 99999999.0, axis=1)

        return other_logit

    def predict(self, x):
        # BG: We don't need this because we use data from tensorflow, not pytorch
    ...
    def get_grad_diff_logits_target(self, x, y=None, y_target=None):
        ...
        #x2 = np.moveaxis(x.cpu().numpy(), 1, 3)
        x2 = x.cpu().numpy()
        dl, g2 = self.sess.run([self.diff_logits, self.grad_diff_logits], {
                               self.x_input: x2, self.la: la, self.la_target: la_target})
    ...
    def get_logits_loss_grad_xent(self, x, y):
        ...
        logits_val, loss_indiv_val, grad_val = self.sess.run(
            [self.logits, self.xent, self.grad_xent], {self.x_input: x2, self.y_input: y2})
        # BG: We don't need this because we use data from tensorflow, not pytorch
    ...
    def get_logits_loss_grad_dlr(self, x, y):
        ...
        y2 = y.clone().cpu().numpy()
        logits_val, loss_indiv_val, grad_val = self.sess.run(
            [self.logits, self.dlr, self.grad_dlr], {self.x_input: x2, self.y_input: y2})
        grad_val = np.moveaxis(grad_val, 3, 1)

        return torch.from_numpy(logits_val).cuda(), torch.from_numpy(loss_indiv_val).cuda(), torch.from_numpy(grad_val).cuda()

    def get_logits_loss_grad_dist(self, x, y):
        #x2 = np.moveaxis(x.cpu().numpy(), 1, 3)
        y2 = y.clone().cpu().numpy()
        logits_val, loss_indiv_val, grad_val = self.sess.run([self.logits, self.dist, self.grad_dist], {
                                                             self.x_input: x.cpu().detach().numpy(), self.y_input: y2})
        #grad_val = np.moveaxis(grad_val, 3, 1)
    ...
    def get_logits_loss_grad_target(self, x, y, y_target):
        ...
        logits_val, loss_indiv_val, grad_val = self.sess.run([self.logits, self.dlr_target, self.grad_target], {
                                                             self.x_input: x2, self.y_input: y2, self.y_target: y_targ})
        #grad_val = np.moveaxis(grad_val, 3, 1)
...
def dlr_loss(x, y, num_classes=10):
    ...
    loss = -(x_sort[:, -1] - x_sort[:, -2]) / \
        (x_sort[:, -1] - x_sort[:, -3] + 1e-12)
...
def dlr_loss_targeted(x, y, y_target, num_classes=10):
    ...
    loss = -(tf.reduce_sum(x * y_onehot, axis=1) - tf.reduce_sum(x * y_target_onehot,
                                                                 axis=1)) / (x_sort[:, -1] - .5 * x_sort[:, -3] - .5 * x_sort[:, -4] + 1e-12)
