...
class SquareAttack():
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if len(x.shape) == 4:  # NCHW
                c, h, w = x.shape[1:]
            elif len(x.shape) == 2:  # ND, madry MNIST
                c = 1
            ...
            if self.norm == 'Linf':
                if len(x.shape) == 2:  # ND, madry MNIST
                    print(x.size())
                    print(self.random_choice(
                        [x.shape[0], c, 1, w]).reshape((x.shape[0], -1)).size())
                    x_best = torch.clamp(x + self.eps * self.random_choice(
                        [x.shape[0], h, 1]).repeat(1, 1, w).reshape((x.shape[0], -1)), 0., 1.)
                else:
                    x_best = torch.clamp(x + self.eps * self.random_choice(
                        [x.shape[0], c, 1, w]), 0., 1.)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    idx_to_fool = torch.arange(margin_min.size(0)).long().cuda()
                    ...
                    if len(x.shape) == 2:  # ND, madry MNIST
                        new_deltas = new_deltas.view(h * w)
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            #ind_to_fool = torch.arange(acc.size(0)).long().cuda()
