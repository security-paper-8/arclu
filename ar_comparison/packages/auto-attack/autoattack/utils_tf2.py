...
class ModelAdapter():
    ...
    def __check_channel_ordering(self):
        ...
        if input_shape[3] == 3:
            ...
            print(
                "[INFO] Because detecting input_shape[3] == 1, set data_format = 'channels_last'")
            self.data_format = 'channels_last'
    ...
    def __get_dist(self, logits, y_input):
        val_dlr = dist_loss(logits, y_input, num_classes=self.num_classes)
        return val_dlr
    ...
    @tf.function
    @tf.autograph.experimental.do_not_convert
    def __get_grad_dist(self, x_input, y_input):
        with tf.GradientTape(watch_accessed_variables=False) as g:
            g.watch(x_input)
            logits = self.__get_logits(x_input)
            val_dlr = self.__get_dist(logits, y_input)
    ...
    def __get_dlr_target(self, logits, y_input, y_target):
        dlr_target = dlr_loss_targeted(
            logits, y_input, y_target, num_classes=self.num_classes)
        return dlr_target
    ...
    def __get_grad_diff_logits_target(self, x, la, la_target):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as g:
            ...
            difflogits = tf.reduce_sum(
                (la_target_mask - la_mask) * logits, axis=1)
    ...
    def get_logits_loss_grad_dist(self, x, y):
        ...
        logits_val, loss_indiv_val, grad_val = self.__get_grad_dist(x2, y2)
    ...
    def get_logits_loss_grad_target(self, x, y, y_target):
        ...
        x2 = tf.convert_to_tensor(x.cpu().numpy(), dtype=tf.float32)
        y2 = tf.convert_to_tensor(y.clone().cpu().numpy(), dtype=tf.int32)
        y_targ = tf.convert_to_tensor(
            y_target.clone().cpu().numpy(), dtype=tf.int32)
        if self.data_format == 'channels_last':
            x2 = tf.transpose(x2, perm=[0, 2, 3, 1])

        logits_val, loss_indiv_val, grad_val = self.__get_grad_dlr_target(
            x2, y2, y_targ)
...
def dist_loss(x, y, num_classes=10):
