...
class APGDAttack():
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        # BG
        additional_shape = [1] * (len(x.shape) - 1)
        if self.norm == 'Linf':
            ...
            # BG
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0]] + additional_shape).to(self.device).detach(
            ) * t / (t.reshape([t.shape[0], -1]).abs().max(dim=1, keepdim=True)[0].reshape([-1] + additional_shape))
        elif self.norm == 'L2':
            ...
            # BG
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0]] + additional_shape).to(self.device).detach(
            ) * t / ((t ** 2).sum(dim=tuple(range(1, len(additional_shape) + 1)), keepdim=True).sqrt() + 1e-12)
        ...
        if self.loss == 'ce':
            ...
            criterion_indiv = self.model.get_logits_loss_grad_dist
        elif self.loss == 'dlr':
            criterion_indiv = self.model.get_logits_loss_grad_dlr
        else:
            raise ValueError('unknowkn loss')
        # BG
        grad = torch.zeros_like(x).float()
        ...
        step_size = self.eps * torch.ones([x.shape[0]] + additional_shape).to(self.device).detach(
        ) * torch.Tensor([0.01]).to(self.device).detach().reshape([1] + additional_shape)
...
class APGDAttack_targeted():
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        grad = torch.zeros_like(x).float()
        for _ in range(self.eot_iter):
            logits, loss_indiv, grad_curr = self.model.get_logits_loss_grad_target(
                x_adv, y, y_target)
            grad += grad_curr
        ...
        #step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1]).to(self.device).detach() * torch.Tensor([2.0]).to(self.device).detach().reshape([1, 1, 1, 1])
        #step_size = self.eps * torch.ones([x.shape[0]]+additional_shape).to(self.device).detach() * torch.Tensor([2.0]).to(self.device).detach().reshape([1]+additional_shape)
        step_size = self.eps * torch.ones([x.shape[0]] + additional_shape).to(self.device).detach(
        ) * torch.Tensor([0.01]).to(self.device).detach().reshape([1] + additional_shape)
        x_adv_old = x_adv.clone()
        counter = 0
